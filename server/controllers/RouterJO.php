        ...
        switch ($_SERVER['REQUEST_METHOD']){
            case 'GET':
                ...
                if (array_key_exists($route, $this->routesGET)){
                    // récupère et instancie le contrôleur pour cette route
                    $class = $this->routesGET[$route];
                    $controller = new $class();
                    // éventuellement on appelle une méthode pour gérer la requête
                    // ou alors c'est fait directement dans le constructeur
                }
                break;
            case 'POST':
            default:
